...
def calculate_metrics(prediction, label):
    if np.min(label) != 0:
        raise ValueError('label is all positive')
...
def validation_MC_dropuout(args, model, test_dataloader, M_dp, image_save_path):
    list_dice = []
    list_IoU = []
    list_sensitivity = []
    list_specificity = []
    ...
    with torch.no_grad():
        for i, test_batch in enumerate(test_dataloader):
            ...
            """Use Jet colormap to visualize variance and save the output."""
...
def inference(args):
    ...
    if args.DP > 0:
        ...
        list_dice, list_IoU, list_sensitivity, list_specificity = validation_MC_dropuout(args, MC_model,
    ...
    if args.DP > 0:
        excel_filename = snapshot_path + '/results_dropout_time_{}.xlsx'.format(args.dp_time)
    else:
        excel_filename = snapshot_path + '/results.xlsx'
