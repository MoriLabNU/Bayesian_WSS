...
from ..pytorch_deeplab_v3_plus.dataloaders.custom_transforms import denormalizeimage
import time
...
class DenseCRFLoss(nn.Module):
    ...
    def forward(self, images, segmentations, ROIs):
        ...
        scaled_images = F.interpolate(images,scale_factor=self.scale_factor,mode='bilinear',align_corners=False)
        scaled_segs = F.interpolate(segmentations,scale_factor=self.scale_factor,mode='bilinear',align_corners=False)
        scaled_ROIs = F.interpolate(ROIs, scale_factor=self.scale_factor)
