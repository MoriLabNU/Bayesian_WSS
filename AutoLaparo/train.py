...
def train(args):
    ...
    train_data = AutoLaparo_loader(work_path=work_path, transform=train_transform, split='train', label_type='scribble')
    ...
    for round_idx in tqdm(range(1, args.max_epochs // 2 + 1), ncols=70):
        ...
        for i, train_batch in enumerate(train_dataloader):
            ...
            loss_pce = F.nll_loss(y_log, label_batch, ignore_index=10)
        ...
        if round_idx % 100 == 0:
            val_L_metric_list = validation_L_model(L_model, val_dataloader)
            ...
            for i in range(len(val_L_metric_list)):
                ...
                logging.info('round_idx %d val_avg of class %d : %f'
                             % (round_idx, i, per_class))

            val_L_metric_average = val_L_metric_average / len(val_L_metric_list)

            logging.info('round_idx %d val_avg : %f' % (round_idx, val_L_metric_average))
            writer.add_scalar('val_avg/', val_L_metric_average, round_idx)
            L_model.train()
    ...
    for round_idx in tqdm(range(1, args.max_epochs // 2 + 1), ncols=70):
        ...
        for i, train_batch in enumerate(train_dataloader):
            ...
            # merge scribble and Y
            mask = torch.zeros_like(y)
            mask[label_batch == 10] = 1
        ...
        if round_idx % 100 == 0:
            val_MC_metric_list = validation(MC_model, val_dataloader)
            ...
            logging.info('round_idx %d val_avg : %f' % (round_idx, val_MC_metric_average))
            writer.add_scalar('val_avg/', val_MC_metric_average, round_idx)
            MC_model.train()
        ...
        if round_idx % 100 == 0:
            ...
            test_MC_metric_list = validation(MC_model, test_dataloader)
            test_MC_metric_average = 0
            for i in range(len(test_MC_metric_list)):
                per_class = np.nanmean(test_MC_metric_list[i])
                test_MC_metric_average += per_class
                logging.info('p(w|x,y)_round_idx %d test_avg of class %d : %f'
                             % (round_idx, i, per_class))

            test_MC_metric_average = test_MC_metric_average / len(test_MC_metric_list)

            logging.info('round_idx %d test_avg : %f' % (round_idx, test_MC_metric_average))
            writer.add_scalar('p(w|x,y)_test_avg/', test_MC_metric_average, round_idx)
            MC_model.train()
...
def parser_args():
    ...
    parser.add_argument('--run', type=int, default=1, help='trail number')
